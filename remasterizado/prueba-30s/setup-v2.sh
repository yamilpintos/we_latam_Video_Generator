#!/bin/bash
# FlashVSR v1.1 Full en Vast, imagen vastai/base-image cuda-12.4.1 devel py311, A100 (sm_80).
# Versión 2 (18/9/2026): con las seis trampas de la primera corrida ya resueltas.
...
git clone --depth 1 https://github.com/OpenImagingLab/FlashVSR || { echo SETUP_FALLO clone; exit 1; }
git clone --depth 1 https://github.com/mit-han-lab/Block-Sparse-Attention || { echo SETUP_FALLO clone-bsa; exit 1; }
# pesos en segundo plano (el binario hf vive en /venv/main/bin)
...
$PY -m pip install -q modelscope "setuptools<80" packaging ninja
$PY -m pip install -q -e . --no-deps --no-build-isolation || { echo SETUP_FALLO pip-e; exit 1; }
# atención dispersa sólo para sm_80; sin la variable mete compute_120 y nvcc 12.4 muere
cd /workspace/Block-Sparse-Attention
export BLOCK_SPARSE_ATTN_CUDA_ARCHS=80 MAX_JOBS=64 NVCC_THREADS=4
T1=$(date +%s)
