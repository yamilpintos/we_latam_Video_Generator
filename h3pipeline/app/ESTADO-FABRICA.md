...
| **ReMusical** (26/9) | la herramienta ENTERA de `herramientas/remusical/` (motor + web + tandas + tests), sin reescribir: `remusical_mount.py` le da el entorno (ELEVENLABS_API_KEY del .env de La Fábrica, FOTON, ffmpeg) y `server.py` la monta en **/remusical/**. Es su propia página (no una vista del router por hash): la puerta y el link de la barra van a `/remusical/`, y ella muestra «← La Fábrica». Flujo: conectar Drive (OAuth propio) → elegir carpeta y videos → mapa de música (AudioSet) → separar sólo esas regiones (BS-Roformer, Python 3.11 aparte o la GPU de Vast) → estilo → takes de Eleven Music → elegir contra la original → montar con ducking → 6 guardas → `re-musical/` en Drive. Pestaña **Tandas**: una serie entera a Vast por cuenta de servicio + bucket. `/api/remusical` dice qué le falta | `herramientas/remusical/remusical/{web,orquestador,tandas_web}.py`, `remusical_mount.py` | probado montada: portada, `/remusical/`, demo, `/remusical/api/config` (callback `…/remusical/auth/callback`), 5 tests pasan. **Falta**: `GOOGLE_CLIENT_ID/SECRET` en `herramientas/remusical/.env` y registrar el callback nuevo en Google Cloud. El proceso en modo `local` corre en la PC (torch); en Render sólo la web y las tandas |
| **Doblaje** (26/9) | la herramienta ENTERA de `herramientas/doblaje/` (web + tests), sin reescribir: `doblaje_mount.py` le da el entorno (ELEVENLABS_API_KEY y VAST_API_KEY del .env de La Fábrica, la clave ssh de Vast, `mis-videos/_doblaje` como trabajo) y `server.py` la monta en **/doblaje/**. Es su propia página: la puerta y el link de la barra van a `/doblaje/`, y ella muestra «← La Fábrica». Flujo: conectar Drive (OAuth propio) → elegir videos, idioma y fuerza de clonado → costo contra el saldo real → ElevenLabs Dubbing v2 (sólo audio) → **nivelación por pistas**: separa original y doblado en una **GPU de Vast que se alquila e instala sola** (`gpu_vast.py`: 4090 ≤ USD 0,60/h, ociosa 10 min o 6 h → se destruye; sólo la instancia con etiqueta `doblaje-app`), voz doblada al nivel de la voz original + fondo original intacto + lo no verbal; mientras separa, ya subió a Drive una versión provisoria por mezcla → verificación (idioma original restante, original filtrado detrás de la voz) → mp4 + informe a `doblaje/` en Drive. **Control de créditos** (`creditos.py`): libro mayor, tope por trabajo y por día, reserva, re-chequeo antes de cobrar, pausa automática si el cobro medido se pasa; panel «Gasto y GPU»; `/api/doblaje` dice qué le falta | `herramientas/doblaje/doblaje/{web,gpu_vast,creditos,media,verificar}.py`, `doblaje_mount.py` | probada montada en local: portada, `/doblaje/`, `/api/doblaje`, `/doblaje/api/config`; 4 suites de tests pasan sin gastar (`tests.correr_todo`). **Falta**: `GOOGLE_CLIENT_ID/SECRET` (sirve el mismo cliente que ReMusical) y registrar `…/doblaje/auth/callback`; la GPU automática NO corrió aún contra una instancia real |
