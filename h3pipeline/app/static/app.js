  ...
  $("#vista").innerHTML = `<div class="wrap"><h1>Nueva serie de ${que}</h1>
    ...
    <div class="grid g2">
      <div class="card"><h3>La ficha</h3>
        ...
        ${f === "short" ? `<div class="tiny" style="margin:-2px 0 8px"><b>Regla de los shorts</b>: una locación, una situación, el personaje a cámara, 1-2 objetos. Se aplica sola en todos los capítulos.</div>` : ""}
...
function pintarSerie(s) {
  ...
  $("#vista").innerHTML = `<div class="wrap"><div id="serie-err">${bloqueo ? `<div class="card" style="margin-bottom:14px;border-color:rgba(255,207,90,.6)"><div class="row"><span class="pill warn">antes de producir</span><span style="flex:1">${h(bloqueo)}</span></div></div>` : ""}</div><h1>${h(s.titulo)} <span class="pill">${FORMATOS_SERIE[s.formato] || s.formato} · ${h(s.estructura)}${s.duracion ? " · " + s.duracion + " s" : ""}</span><span class="pill">${s.continuidad === "serial" ? "serial" : "antología"}</span>${s.toma === "una" ? `<span class="pill ac">${Math.round((s.duracion || 15) / 15.083)} toma(s) de 15 s · ${Math.round(s.duracion || 15)} s</span>` : ""}${s.modo === "actuado" ? `<span class="pill ac">actuado · hablan los personajes</span>` : s.voz ? `<span class="pill">narrado · voz ${h(s.voz)}</span>` : s.formato === "musica" ? "" : `<span class="pill">sin voz</span>`}<a class="btn s" href="${volver}" style="margin-left:auto">volver a ${FORMATOS_SERIE[s.formato] || s.formato}</a></h1>
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      ${s.formato === "short" ? `<div class="pill" style="display:block;white-space:normal;line-height:1.4;padding:8px 12px;margin:4px 0 8px;color:var(--tx2)"><b style="color:var(--ac)">Regla de los shorts</b> (fija, la aplican el plan, el guion y el traductor): cada capítulo pasa en <b>una sola locación</b>, con <b>una sola situación</b> y el personaje <b>hablando a cámara</b> desde un encuadre fijo; humanos secundarios sólo en off o desenfocados; 1-2 objetos apoyados. Simple y repetible: cuanto más lugares y más trama, más incontrolable.</div>` : ""}
