...
function pintarSerie(s) {
  ...
  $("#vista").innerHTML = `<div class="wrap"><div id="serie-err">${bloqueo ? `<div class="card" style="margin-bottom:14px;border-color:rgba(255,207,90,.6)"><div class="row"><span class="pill warn">antes de producir</span><span style="flex:1">${h(bloqueo)}</span></div></div>` : ""}</div><h1>${h(s.titulo)} <span class="pill">${FORMATOS_SERIE[s.formato] || s.formato} · ${h(s.estructura)}${s.duracion ? " · " + s.duracion + " s" : ""}</span><span class="pill">${s.continuidad === "serial" ? "serial" : "antología"}</span>${s.toma === "una" ? `<span class="pill ac">${Math.round((s.duracion || 15) / 15.083)} toma(s) de 15 s · ${Math.round(s.duracion || 15)} s</span>` : ""}${s.modo === "actuado" ? `<span class="pill ac">actuado · hablan los personajes</span>` : s.voz ? `<span class="pill">narrado · voz ${h(s.voz)}</span>` : s.formato === "musica" ? "" : `<span class="pill">sin voz</span>`}${s.formato === "largo" && s.voz && s.modo !== "actuado" ? `<span class="pill" title="las citas entre comillas las dicen los personajes en cámara, con su voz del banco"><label><input type="checkbox" ${s.dialogos !== false ? "checked" : ""} onchange="serieCampo('${slug}',{dialogos:this.checked})"> diálogos en cámara</label></span>` : ""}${s.formato === "largo" ? `<span class="pill" title="duración de cada capítulo: la estructura se estira a estos minutos">duración <input id="s-dur-min" type="number" min="1" max="17" step="0.5" value="${Math.round(((s.duracion || 300) / 60) * 2) / 2}" style="width:56px;padding:2px 6px;font-size:12px" onchange="serieDuracion('${slug}',this.value)"> min</span>` : ""}<a class="btn s" href="${volver}" style="margin-left:auto">volver a ${FORMATOS_SERIE[s.formato] || s.formato}</a></h1>
    ...
    <div class="card" style="margin-bottom:14px">
      ...
      ${s.formato === "short" ? `<div class="pill" style="display:block;white-space:normal;line-height:1.4;padding:8px 12px;margin:4px 0 8px;color:var(--tx2)"><b style="color:var(--ac)">Regla de los shorts</b> (fija, la aplican el plan, el guion y el traductor): historias <b>muy simples</b>: como máximo <b>una escena por toma de 15 s</b> (15 s = 1, 30 s = hasta 2, 45 s = hasta 3, 60 s = hasta 4; menos es mejor), <b>una sola situación</b>, el personaje <b>hablando a cámara</b> desde un encuadre fijo; humanos secundarios sólo en off o desenfocados; 1-2 objetos apoyados. Cuanto más lugares y más trama, más incontrolable.</div>` : ""}
...
async function serieCampo(slug, campos) {
  try { await api(`/series/${slug}`, {method: "PUT", body: campos}); toast("guardado"); navegar(); } catch (e) { serieError(e.message); }
}
async function serieDuracion(slug, minutos) {
  const seg = Math.max(60, Math.round(Number(minutos) * 60));
  try { await api(`/series/${slug}`, {method: "PUT", body: {duracion: seg}}); toast(`capítulos de ${minutos} min: el guion y los planos se miden a eso`); navegar(); } catch (e) { serieError(e.message); }
}
